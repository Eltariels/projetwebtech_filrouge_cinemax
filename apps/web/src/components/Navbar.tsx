...
export default function Navbar() {
    ...
    return (
        <nav style={{
            ...
            {/* Droite */}
            <div style={{ ...s.right, marginLeft: "auto", gap: isMobile ? "0.5rem" : "1rem" }}>
                ...
                {user ? (
                    <>
                        ...
                        {compact && (
                            <button onClick={() => { logout(); navigate("/"); }} style={{ ...s.logoutBtn, padding: "0.35rem 0.6rem", fontSize: "0.75rem" }}>
                                ⏻
                            </button>
                        )}
                    </>
...
const s: Record<string, React.CSSProperties> = {
    ...
    searchForm: { flex: 1 },
    searchInput: { width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid var(--border)", borderRadius: 4, padding: "0.5rem 1rem", color: "var(--text)", fontSize: "0.875rem", outline: "none" },
    ...
    btnGold: { background: "linear-gradient(135deg, var(--gold-dark), var(--gold))", color: "#0a0a0a", fontWeight: 600, borderRadius: 4, flexShrink: 0 },
